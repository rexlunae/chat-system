        ...
        while let Ok((mut stream, _)) = listener.accept().await {
            tokio::spawn(async move {
                ...
                let (status, body) = if request_line.starts_with("POST /_matrix/client/v3/login ") {
                    (
                        200,
                        r#"{"access_token":"test-token","user_id":"@bot:example.com"}"#,
                    )
                    ...
                    && request_line.contains("since=s1")
                {
                    (
                        200,
                        r#"{"next_batch":"s2","rooms":{"join":{"!room:example.com":{"timeline":{"events":[{"type":"m.room.message","event_id":"$event-1","sender":"@alice:example.com","origin_server_ts":1712000000000,"content":{"body":"hello from matrix"}}]}}}}}"#,
                    )
                } else if request_line.starts_with("GET /_matrix/client/v3/sync?") {
                    (200, r#"{"next_batch":"s1","rooms":{"join":{}}}"#)
                } else if request_line.starts_with("POST /_matrix/client/v3/join/") {
                    (200, r#"{"room_id":"!room:example.com"}"#)
...
async fn matrix_initialize_sets_connected() {
    ...
    let mut m = MatrixMessenger::new("matrix", homeserver, "@bot:example.com", "secret");
...
async fn matrix_send_message_joins_alias_and_returns_event_id() {
    ...
    let mut m = MatrixMessenger::new("matrix", homeserver, "@bot:example.com", "secret");
    m.initialize().await.unwrap();
    let id = m.send_message("#room:example.com", "hello matrix").await.unwrap();
...
async fn matrix_receive_messages_returns_synced_messages() {
    ...
    assert_eq!(messages[0].channel.as_deref(), Some("!room:example.com"));
}
